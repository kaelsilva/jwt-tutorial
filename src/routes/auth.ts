...
router.get('/login', (req, res) => {
	User.findOne({ email: req.body.email })
		.then((user: any) => {
			if (!user) res.status(404).json({ error: 'No user found with this e-mail.' })
			else {
				bcrypt.compare(req.body.password, user.password, (error:any, match:any) => {
...
router.post('/signup', (req:any, res:any) => {
	bcrypt.hash(req.body.password, rounds, (error:any, hash:any) => {
		if (error) {
